...
for selectCateg in allCateg :
	...
	WHERE Produit.Categorie_idCategorie=%s AND Commande.Livraison BETWEEN '%s 00:00:00' AND '%s 23:59:59'
	;
	""" % (selectCateg, params["date"], params["date"]) #MAGmenu: """AND Magasin.idMagasin=%s""" , params["magasin"] 
	db.execute(request)
	...
	INNER JOIN Magasin
	ON Terminal.Magasin_idMagasin=Magasin.idMagasin
	WHERE ProduitCustom.Categorie_idCategorie=%s AND Commande.Livraison BETWEEN '%s 00:00:00' AND '%s 23:59:59'
	;
	""" % (selectCateg, params["date"], params["date"]) #MAGmenu: """AND Magasin.idMagasin=%s""" , params["magasin"] 
	db.execute(request)
